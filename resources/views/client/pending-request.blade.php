                        ...
                        <div class="wrap-list-item-fluid">
                            ...
                            <div class="list-item-post">
                                @foreach($listRequest as $item)
                                    <div class="item-post">
                                        <a href="/detail/{{$item->myTrade->id}}" class="item-image">
                                            <img src="{{ $item->myTrade->firstImg }}" style="max-width: 100%" alt="">
                                        </a>
                                        <div class="item-info">
                                            <h3>
                                            ...
                                            <p style="font-size: 15px">Đăng bởi : {{ $item->myTrade->account->full_name }}  </p>

                                            <p class="location">
                                                {{ $item->myTrade->address }}, {{ $item->myTrade->wardName->name }}
                                                ,{{ $item->myTrade->districtName->name }},{{ $item->myTrade->cityName->name }}
                                            </p>
